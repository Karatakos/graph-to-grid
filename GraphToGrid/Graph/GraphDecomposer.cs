...
public static class GraphDecomposer
{
    public static GraphDecomposition Generate(UndirectedAdjacencyListGraph<Vertex> graph) { 
        ...
        for (int i=0; i<facestmp.Faces.Count; i++) {
            G2GDebug.Logger.LogInformation(String.Format("Detected face {0}", i+1));
            for (int j=0; j<facestmp.Faces[i].Count; j++) {
                G2GDebug.Logger.LogInformation(String.Format("{0}: {1}",
                    facestmp.Faces[i][j],
            ...
            G2GDebug.Logger.LogInformation("");
        }
        ...
        for (int i=0; i<decomp.Chains.Count; i++) {
            G2GDebug.Logger.LogInformation(String.Format("Extracted chain {0}", i+1));
            G2GDebug.Logger.LogInformation(String.Format("Vertices: {0}",
