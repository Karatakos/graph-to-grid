...
public class Line {
    ...
    public static bool TryGetOverlappingLine(Line l1, Line l2, out Line contact) {
        ...
        //
        if ((l1Max.x < l2Min.x - G2GConfig.Tolerance || l1Min.x > l2Max.x + G2GConfig.Tolerance) ||
            (l1Max.y < l2Min.y - G2GConfig.Tolerance || l1Min.y > l2Max.y + G2GConfig.Tolerance))
            return false;

        // They're overlapping but are they colinear? If not then ignore
        //
        if (Math2D.PointToLineSqDistance(l2.Start, l1) > G2GConfig.Tolerance ||
            Math2D.PointToLineSqDistance(l2.End, l1) > G2GConfig.Tolerance)
            return false;
    ...
    public static List<Line> Union(List<Line> lines) {
        ...
        foreach (Line newLine in lines.OrderBy(x => x.GetLength2()).ToList()) {
            ...
            foreach (Line existingLine in union) {
                ...
                if (Math.Abs(cross) > G2GConfig.Tolerance)
                    continue;
                ...
                //
                if (ds < G2GConfig.Tolerance || de < G2GConfig.Tolerance)
                {
    ...
    public static float ComputeLineContactArea(Line l1, Line l2) {
        ...
        if (Math.Abs(l1xl2Mag2) > G2GConfig.Tolerance)
            return 0F;
        ...
        if (d1 > G2GConfig.Tolerance || d2 > G2GConfig.Tolerance)
            return 0F;
