...
public class ConfigSpacesBuilder {
    ...
    public bool TryFromRoom(Room fixedRoom, Room movingRoom, out ConfigSpace configSpace) {
        ...
        float contactThreshold = G2GConfig.DoorWidth + (G2GConfig.DoorToCornerMinGap * 2);
            ...
            foreach (BoundaryLine mline in movingRoom.Boundary) {
                ...
                for (int k=1; k<pos.Length; k++) {
                    ...
                    for (int j=0; j<testPoints.Count(); j++) {
                        ...
                        if (contact < contactThreshold - G2GConfig.Tolerance) {
    ...
    public ConfigSpace FromConfigSpace(ConfigSpace spaceA, ConfigSpace spaceB) {
        ...
        foreach (Line l1 in spaceA.Lines) {
            foreach (Line l2 in spaceB.Lines) {
                // Points not lines 
                //
                if (l1.GetLength2() < G2GConfig.Tolerance && l2.GetLength2() < G2GConfig.Tolerance) {
                    // If points overlap then add either! 
                    //
                    if (Vector2F.Magnitude2(l1.Start - l2.Start) < G2GConfig.Tolerance)
                        clines.Add(l1);
                ...
                //  
                if (l1.GetLength2() < G2GConfig.Tolerance) {
                    if (Math2D.PointToLineSegmentSqDistance(l1.Start, l2) < G2GConfig.Tolerance)
                        clines.Add(l1);
                ...
                //
                if (l2.GetLength2() < G2GConfig.Tolerance) {
                    if (Math2D.PointToLineSegmentSqDistance(l2.Start, l1) < G2GConfig.Tolerance)
                        clines.Add(l2);
                ...
                //
                if (l1xl2Mag2 > G2GConfig.Tolerance) {
