...
public class Renderer : Game
{
    ...
    protected override void Initialize()
    {
        ...
        LayoutGraph graph = InitializeDungeon();
        _dungeon = GenerateLayout(graph);

        base.Initialize();
    }

    private LayoutGraph InitializeDungeon() {
        float x = 50/2;
        float y = 30/2;
    ...
    private Layout GenerateLayout(LayoutGraph graph) {
        Layout layout = null;
    
        try {
            G2GConfig.DoorWidth = _doorWidth;
            G2GConfig.DoorToCornerMinGap = _doorGapMin;
            ...
            var generator = new LayoutGenerator(graph);
